...
def _parse_table_concepts(section: str) -> dict:
    """从标准表格形式的关键概念中提取概念（表头含多列）。"""
    concepts = {}
    lines = section.split('\n')
    header_idx = -1
    for i, line in enumerate(lines):
        if '|' in line and '术语名称' in line and '通俗解释' in line:
            header_idx = i
            break
    ...
    term_idx = col_index('术语名称')
    def_idx = col_index('文中定义')
    expl_idx = col_index('解释')
    lay_idx = col_index('通俗解释')
    rel_idx = col_index('关联概念')
    ...
    for line in lines[header_idx + 2:]:
        ...
        if cells and not cells[0]:
            cells = cells[1:]
        if not cells or len(cells) < term_idx + 1:
            continue

        term = _clean_md(cells[term_idx])
        if not term:
            continue
        ...
        concept = {
            'title': term,
            'term': term,
            'definition': _clean_md(cells[def_idx]) if def_idx >= 0 and def_idx < len(cells) else '',
            'explanation': _clean_md(cells[expl_idx]) if expl_idx >= 0 and expl_idx < len(cells) else '',
            'layman': _clean_md(cells[lay_idx]) if lay_idx >= 0 and lay_idx < len(cells) else '',
            'related': _clean_md(cells[rel_idx]) if rel_idx >= 0 and rel_idx < len(cells) else '',
        }

        key = term.split('（')[0].strip() if '（' in term else term.split('(')[0].strip() if '(' in term else term
        key = key.replace('*', '')
        concept['key'] = key
...
def _parse_keyvalue_table_concepts(section: str) -> dict:
    """从键值对表格形式的关键概念中提取概念（表头为项目/内容）。"""
    concepts = {}
    lines = section.split('\n')
    i = 0
    while i < len(lines):
        ...
        if title_match:
            ...
            i += 1
            concept = {'title': title, 'term': '', 'definition': '', 'explanation': '', 'layman': '', 'related': ''}
            while i < len(lines):
                row = lines[i].strip()
                if not row or row.startswith('###') or row.startswith('##'):
                    break
                if '|' in row and '项目' in row and '内容' in row:
                    i += 2  # 跳过表头和分隔行
                    while i < len(lines):
                        kv_row = lines[i].strip()
                        if not kv_row or kv_row.startswith('|--') or kv_row.startswith('|---'):
                            i += 1
                            continue
                        if not kv_row.startswith('|'):
                            break
                        cells = [c.strip() for c in kv_row.split('|')]
                        ...
                        if len(cells) >= 2:
                            ...
                            if '术语' in key:
                                ...
                                concept['definition'] = val
                            elif '解释' == key or '解释' in key:
                                concept['explanation'] = val
                            elif '通俗' in key:
                                concept['layman'] = val
                            elif '关联' in key:
                                concept['related'] = val
...
def parse_concepts_from_report(report_path: str) -> dict:
    ...
    # 找到 "## 关键概念与定义" 部分（兼容 "## 3. 关键概念与定义" 等编号前缀）
    ...
    # 如果该部分包含表格形式的概念，优先用表格解析
    if '|' in section and '术语名称' in section and '通俗解释' in section:
        if re.search(r'\|\s*术语名称\s*\|.*?\|\s*通俗解释\s*\|', section):
            return _parse_table_concepts(section)
        if re.search(r'\|\s*项目\s*\|.*?\|\s*内容\s*\|', section):
            return _parse_keyvalue_table_concepts(section)

    # 匹配每个概念块（兼容 "### 概念1:"、"### 3.1 标题" 等常见格式）
