...
check_python_deps() {
    if python3 -c "import fitz" 2>/dev/null; then
        echo "[OK] PyMuPDF (fitz) 已安装"
    else
        echo "[WARN] PyMuPDF (fitz) 未安装，运行需要: pip install PyMuPDF"
    fi

    if python3 -c "import pytesseract" 2>/dev/null; then
        echo "[OK] pytesseract 已安装"
    else
        echo "[INFO] pytesseract 未安装，OCR 功能不可用"
    fi
}
...
check_file "scripts/split_book.py" "拆分脚本"
