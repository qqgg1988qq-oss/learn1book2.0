...
description: |
  ...
  输出：{book-name}/ 目录，包含拆分后的章节 + 每章的精读报告 + 总索引。交互式阅读页面为可选步骤。
...
> **此步骤为可选增强功能。** 如果不需要交互式 HTML 阅读页面，可直接跳过此步骤，进入 Obsidian 同步。

调用 `/book-reader` 为已完成的精读报告生成交互式 HTML 阅读页面：

```bash
python3 "{CLAUDE_SKILL_DIR}/book-reader/scripts/generate.py" "{book-name}"
```

此步骤将：
- 解析所有章节报告（`reports/`）中的关键概念与通俗解释
- 在原文章节文本中标注概念高亮
- 生成左栏原文 + 右栏概念卡片的交互式阅读页面
- 输出到 `{book-name}/reader/` 目录

**何时跳过：**
- 不需要在浏览器中阅读交互式页面
- 只需要 Markdown 格式的精读报告和总索引
- 计划仅在 Obsidian 中阅读
...
├── reader/                        # book-reader 输出（可选，交互式阅读页面）
│   ├── index.html
│   ├── chapter-01.html
│   ├── chapter-02.html
│   └── ...
...
- `/book-reader` 是可选增强步骤，需要已完成 `/deep-reader` 输出的 `reports/` 目录
- 交互式阅读页面为纯静态 HTML，无需服务器，直接用浏览器打开 `reader/index.html` 即可
